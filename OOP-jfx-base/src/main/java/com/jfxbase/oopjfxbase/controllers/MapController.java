...
public class MapController extends SceneController {
    ...
    @FXML
    private RadioButton sortByNumbers;
    ...
    @FXML
    private void handleSortByNumbers() {
        sortEthnicities(true); // true indicates sorting by numbers
    }

    @FXML
    private void handleSortByName() {
        sortEthnicities(false); // false indicates sorting by name
    }

    private void sortEthnicities(boolean sortByNumbers) {
        // Implement the sorting logic here

        List<Label> ethnicityLabels = new ArrayList<>();
        GridPane ethnicitiesGrid = (GridPane) romaniaMap.getParent().lookup("#ethnicitiesGrid");

        for (Node node : ethnicitiesGrid.getChildren()) {
            if (node instanceof Label) {
                ethnicityLabels.add((Label) node);
            }
        }

        if (sortByNumbers) {
            // Sort by numbers logic
            Collections.sort(ethnicityLabels, getEthnicityLabelComparator());
        } else {
            // Sort by name logic
            Collections.sort(ethnicityLabels, Comparator.comparing(Label::getText));
        }

        // Clear the grid and add the sorted labels back
        ethnicitiesGrid.getChildren().clear();
        for (int i = 0; i < ethnicityLabels.size(); i++) {
            ethnicitiesGrid.add(ethnicityLabels.get(i), i % 2, i / 2);
        }
    }

    private void initializeDropdowns() {
        List<String> countyNames = new ArrayList<>(initialCountyData.keySet());
        Collections.sort(countyNames);  // Sort the county names alphabetically
        countyDropdown.getItems().addAll(countyNames);

        ethnicityDropdown.getItems().addAll("ro", "ma", "romi", "ucr", "ger", "tu", "ru", "sb", "slo", "bu", "gr", "ev", "ce", "pol", "arm", "other", "unavailable");
    }

    private Comparator<Label> getEthnicityLabelComparator() {
        return Comparator.comparingDouble(label -> {
    ...
    public void initialize() {
        ...
        // Initialize toggle group for sorting radio buttons
        sortToggleGroup = new ToggleGroup();
        sortByNumbers.setToggleGroup(sortToggleGroup);
        sortByName.setToggleGroup(sortToggleGroup);

        // Default sorting by numbers
        sortByNumbers.setSelected(true);
    ...
    private void displayEthnicData(String countyName) {
        if (countyData.containsKey(countyName)) {
            ...
            Comparator<Label> comparator = getEthnicityLabelComparator();
            ...
            for (int i = 0; i < ethnicityPopulation.length; i++) {
                ...
                ethnicityLabels.add(ethnicityLabel);
            }

            // Sort the labels based on the selected criteria
            ethnicityLabels.sort(comparator);

            // Add the sorted ethnicity labels to the GridPane
            for (int i = 0; i < ethnicityLabels.size(); i++) {
                ethnicitiesGrid.add(ethnicityLabels.get(i), i % 2, i / 2);
            }
        }
